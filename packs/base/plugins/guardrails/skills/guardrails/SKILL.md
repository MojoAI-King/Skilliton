...
These switch hooks off without touching the rules, so they ask too: a shell command that writes the client's settings file (`.claude/settings.json` or `.claude/settings.local.json`, spelled with `./`, `$PWD/` or an absolute path too, by a redirection, a heredoc, `tee`, `cp` or `mv` onto it, `sed -i`, or a `python3 -c` or `node -e` naming it) when the command's text, the file as it is now, or a file copied onto it holds `hooks`, `disableAllHooks` or `enabledPlugins`; and creating the opt-out file `.skilliton-off` (or `skilliton-off` inside the Git folder) by any command, because while it is there every workflow hook stays silent. A settings write that holds none of those words, such as a permissions change in a file with no hooks, goes through. Keeping a repository out of Skilliton, or turning hooks off, is a person's decision.
