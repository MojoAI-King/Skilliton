...
import { ConfigError, formatRecordHeader, resolveProject } from './config.mjs';
...
// ---------- findings: their own file, and one line in the backlog record (B78) ----------
...
export const DEFAULT_FINDINGS_FILE = 'docs/SECURITY_FINDINGS.md';

// security.findingsFile in .skilliton/config.json, else docs/SECURITY_FINDINGS.md: a repository-relative .md path
// that is not the backlog record. The value is never echoed in the refusal.
export function findingsFileRel(project) {
  const rel = project.security?.findingsFile ?? DEFAULT_FINDINGS_FILE;
  const detail = 'security.findingsFile in .skilliton/config.json must be a repository-relative .md path other than the backlog record';
  if (typeof rel !== 'string' || !rel.endsWith('.md') || rel === project.artifacts.backlog) fail('UNSAFE_PATH', detail);
  try { relativePath(rel); } catch { fail('UNSAFE_PATH', detail); }
  return rel;
}
...
// The generated section of the findings file, between the two marker lines.
export function renderFindingsBlock(ev, findings) {
  const lines = [FINDINGS_START, '## Open findings', '',
    `Generated by skilliton security findings from ${SECURITY_DIR} (catalog ${md(ev.catalog.catalogVersion)}). `
    + 'Do not edit between the marker lines: record evidence, then run the command again. Text outside them is never changed.', ''];
...
// What the backlog record keeps between its own markers: one line, a link to the findings file with the count.
export function renderBacklogLink(backlogRel, findingsRel, count) {
  const from = backlogRel.includes('/') ? backlogRel.slice(0, backlogRel.lastIndexOf('/')) : '.';
  const link = posix.relative(from, findingsRel);
  return [FINDINGS_START, `Security findings: ${count} open, listed in [${md(findingsRel)}](${link}) `
    + '(generated by skilliton security findings; do not edit between these markers).', FINDINGS_END].join('\n');
}

// The findings file as first written: its title and record header, then the generated section. latin1 in and out.
export function newFindingsFile(project, block) {
  const head = Buffer.from(`# Security findings\n\n${formatRecordHeader(project.recordHeader, 'Reference.')}\n`, 'utf8').toString('latin1');
  return upsertFindings(head, block);
}
...
// { rel, exists, bytes, before, fileMode } for a managed file; before is '' when it does not exist yet.
function readManaged(root, rel, budget) {
  const stat = checkedPath(root, rel, true).stat;
  if (!stat) return { rel, exists: false, bytes: null, before: '', fileMode: 0o644 };
  const bytes = safeRead(root, rel, LIMIT.backlog, budget);
  return { rel, exists: true, bytes, before: bytes.toString('latin1'), fileMode: stat.mode & 0o777 };
}

const parentOf = (rel) => (rel.includes('/') ? rel.slice(0, rel.lastIndexOf('/')) : '');

// Reads only. plan.files is the findings file then the backlog record, each with the bytes read, so writeFindings
// refuses when either changed in between. plan.rel is the findings file; plan.changed says whether a write is due.
export function planFindings(root) {
  const budget = newBudget();
  const ev = evaluateSecurity(root, { budget });
  const findings = openFindings(ev);
  const backlogRel = ev.project.artifacts.backlog;
  const rel = findingsFileRel(ev.project);
  if (!checkedPath(root, backlogRel, true).stat) fail('MISSING_BACKLOG', `configured backlog record: ${backlogRel}`);
  const parent = parentOf(rel);
  if (parent && !inspect(root, parent, true)?.isDirectory()) fail('UNSAFE_DIRECTORY', 'the folder of the findings file does not exist');
  const file = readManaged(root, rel, budget), backlog = readManaged(root, backlogRel, budget);
  const block = renderFindingsBlock(ev, findings);
  file.after = file.exists ? upsertFindings(file.before, block) : newFindingsFile(ev.project, block);
  backlog.after = upsertFindings(backlog.before, renderBacklogLink(backlogRel, rel, findings.length));
  const files = [file, backlog];
  return { rel, backlogRel, files, changed: files.some((f) => f.before !== f.after), findings, evaluation: ev };
}

// Both files are checked against the bytes the plan read before either is written, so a file someone changed
// meanwhile stops the write whole; each write then rechecks its own file immediately before it is placed.
export function writeFindings(root, plan) {
  const due = plan.files.filter((f) => f.before !== f.after);
  for (const f of due) {
    const now = checkedPath(root, f.rel, true).stat ? safeRead(root, f.rel, LIMIT.backlog, newBudget()) : null;
    if (f.exists ? !now || !now.equals(f.bytes) : now !== null) fail('CHANGED_SINCE_READ');
  }
  for (const f of due) {
    publish(root, f.rel, Buffer.from(f.after, 'latin1'), 'replace', { expected: f.exists ? f.bytes : null, tempDir: parentOf(f.rel), fileMode: f.fileMode });
  }
}
