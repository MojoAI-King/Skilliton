// prototype-v1.mjs: what the standalone prototype (layout 1, commit 23aae41) wrote into a prepared project, frozen so
...
export const PROTOTYPE_COMMIT = "23aae41";
...
// sha256 of scripts/security-evidence.mjs at commit 23aae41 (the prototype runtime release), computed once from
...
// The prototype's managed block markers (scripts/prepare.mjs at 23aae41, markerStart and markerEnd).
export const PROJECT_MARKER_START = '<!-- skillgate:project:start v1 -->';
export const PROJECT_MARKER_END = '<!-- skillgate:project:end -->';

// How the prototype framed a managed block's content (scripts/prepare.mjs at 23aae41, inside managed()).
export function prototypeBlock(content) {
  const markerStart = PROJECT_MARKER_START, markerEnd = PROJECT_MARKER_END;
  return `${markerStart}\n${content.trimEnd()}\n${markerEnd}`;
}

// Exact copies of the prototype renderers (scripts/project-files.mjs at 23aae41). projectInstructions(artifacts) was
