// commands/remove.mjs: `skilliton remove` (docs/CONTRACTS.md section 10). Takes out what Skilliton manages in a
...
export const help = `remove: take Skilliton's managed content out of a project, keeping its records and history.
  ...
  remove --apply --config [--dir <repo root>]   also delete .skilliton/config.json

Removes the harness block from CLAUDE.md and AGENTS.md (with one blank line next to it; the rest of each file stays
byte for byte), and .skilliton/security/REPORT.md when its first line is the generated-report marker (a report
without the marker may be a person's work and is kept). With --config, also .skilliton/config.json.
...
--apply takes .skilliton/prepare.lock and backs up every file it changes into <git folder>/skilliton-backups/<id>/.
...
export async function run(argv) {
  try {
    ...
    if (project.legacyNames) {
      const what = project.layoutVersion === 1 ? "layout 1 (the standalone prototype), whose copied runtime and skillgate:project blocks remove does not handle" : `layout ${project.layoutVersion ?? 2} under the earlier ${LEGACY_NAME} names (${LEGACY_PROJECT_DIR}/), whose files remove does not handle`;
      refuse(`this project uses ${what}. Migrate it first: ${selfCommand()} migrate --dir ${argPath(root)}, then run remove again. Nothing was written`);
    }
    ...
    const observations = countFiles(root, ".skilliton/security/records", (n) => n.endsWith(".json"));
    ...
    const kept = [
      ...
      `the security register (${inspectPath(root, CATALOG_REL).exists ? "catalog" : "no catalog"}, ${inspectPath(root, ".skilliton/security/applicability.json").exists ? "applicability decisions" : "no applicability file"}, ${observations} observation record(s))`,
    ...
    say(`skilliton remove (${mode}): ${tilde(root)}`);
    say("");
    ...
    if (e instanceof OperationFailed) { console.error(`skilliton: remove could not run: ${e.message}`); return 3; }
