// commands/new-plugin.mjs: `skillgate new-plugin`, which creates a plugin for a company's own skills and lists it where
...
such as MIT to share it. Without --description the description is a TODO(skillgate) placeholder to replace.
...
Preview by default; --apply writes the manifest and backs up the two shared files under $SKILLGATE_BACKUPS/new-plugin/
before changing them. --repo defaults to the skills repository this copy of skillgate is in.
...
export function run(argv) {
  ...
  say(`skillgate new-plugin${o.apply ? "" : " (preview; nothing written)"}`);
  ...
  say(`${plan.placeholder ? "Replace the TODO(skillgate) description in plugin.json and the catalog, then c" : "C"}heck the plugin with: claude plugin validate --strict ${plan.rel}, and commit.`);
  return 0;
}
