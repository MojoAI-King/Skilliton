// commands/verify.mjs: `skilliton verify`, installed plugins against approved company releases. Writes nothing.
...
skills repository this copy of skilliton runs from, when there is one; otherwise the clone recorded by
skilliton join for --company, or for the only company that joined this machine). A release counts only when its tag
skilliton-release/<version> carries an SSH signature that git verify-tag accepts against the company's trust file
(see: skilliton trust --help), and the manifest at the tagged commit has the signed manifest-sha256.
...
Each installed plugin gets one line:
  ...
  WITHDRAWN        its files match a release withdrawn by a signed skilliton-withdrawn/<version> tag
...
--json prints exactly one JSON object (schema skilliton.result/1) and nothing else.
...
function printJson(result, summary, details) {
  process.stdout.write(`${JSON.stringify({ schema: "skilliton.result/1", command: "verify", result, summary, details }, null, 2)}\n`);
}
...
export async function run(argv) {
  ...
  try {
    ...
    if (json) printJson(report.result, report.summary, report.details);
      ...
      say("skilliton verify (writes nothing)");
