...
Run `skilliton security status`. Explain the result in plain words:
...
If `skilliton` is not found on the shell path, say so and run the plugin's own copy by its path: `bin/skilliton` in the workflow plugin folder, two folders above this skill's base directory. If the project has no security register, say so and offer `skilliton prepare` (it adds the register). Do not improvise one.

## 2. Decide what applies, with a named person

For each undecided control, ask the user (or the person they name) whether it applies to this project and why. Record their answer, never yours: `skilliton security applicability --control <id> --applies true|false --rationale "<their reason>" --decided-by "<their name or role>" --apply`. A "does not apply" needs a real reason; it leaves the count visibly, not silently.

## 3. Gather real evidence

- Use a collector when one fits: `skilliton security collect tests --source <file the tests cover> --apply` runs the checks named in `.skilliton/delivery.json`; `skilliton security collect secrets --apply` scans tracked files for secret-shaped text; `skilliton security collect delivery-policy --apply` checks that a delivery policy exists. Each stores its raw output under `.skilliton/private-evidence/` (not committed) and records the result, including a gap when the check fails.
- Otherwise record what a person or a tool actually did: `skilliton security record --control <id> --assessment observed|gap|needs-human --source <file assessed> --artifact <real output file> --note "<what was checked and what it showed>" --reviewer "<who>" --apply`. "Observed" requires real source and output files. Never write an observation for something that did not run.
- Keep sensitive outputs (logs with customer data, credentials) out of the repository: redact them or keep them in `.skilliton/private-evidence/`. Never paste a secret value anywhere.
...
- On an integration branch, `skilliton security findings --apply` turns every open gap into one backlog row (it never duplicates rows and removes a row once a later observation resolves it).
