...
**3. Main window:** say "set up the lanes" (it only creates branches and folders; nothing is deleted), or paste each lane's setup lines into the terminal yourself.

**4. Each lane:** open the lane's folder in a new window, start a **fresh** conversation, and paste the first sentence printed in `LANES.md`. It is the same sentence for every lane and every batch: it tells the session to find its own brief and follow it. Leave it alone until it says **LANE DONE**.
