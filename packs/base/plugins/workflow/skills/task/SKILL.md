...
- Run `skillgate status`. It shows the current task for this branch, open tasks, and whether the last session ended without a handoff. If `skillgate` is not found on the shell path, say so and run the plugin's own copy by its path: `bin/skillgate` in the workflow plugin folder, two folders above this skill's base directory. Only if that is missing too, continue with the same steps by hand and say the records were written by hand.
- If a current task matches the request, read it (`skillgate task show`): its acceptance criteria, its last checkpoint, and its handoff. Tell the user in two sentences where it stands, then continue from its "Next".
...
- Create the record: `skillgate task start "<title>" --criteria "<criterion>" --criteria "<criterion>" --apply`. It lands in the project's task folder with a collision-free id, so two people starting tasks at once never overwrite each other.
...
- After each step that decided, verified or blocked something, record a checkpoint: `skillgate checkpoint --state "<what is true now>" --evidence "<what ran and what it showed>" --next "<the next step>" --apply`. The stop hook reminds you when there are changes and no recent checkpoint; do not wait for it.
- A choice someone might later question (a library, a data shape, a trade-off) gets its own entry: `skillgate record decision "<title>" --apply`, then fill in why and what else was considered.
- Evidence is only what actually ran in this session. "Should work" is not evidence.
- Never edit a list between `skillgate:index` markers by hand. If `skillgate index` cannot run, say why and leave the list for the next run.
...
- Set the real state: `skillgate task close <id> --state done-local --apply` when it is finished on this branch. `merged`, `released` and `verified` are later states that need their own evidence; never skip ahead.
