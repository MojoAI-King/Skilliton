...
- Run `skillgate status`. It shows the current task for this branch, open tasks, and whether the last session ended without a handoff. If `skillgate` is not found, say the workflow plugin is not active here and continue with the same steps by hand.
