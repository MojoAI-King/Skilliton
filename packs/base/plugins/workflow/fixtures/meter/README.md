...
Synthetic transcripts with hand-computed expected totals, so the meter's correctness is checkable by anyone, independent of the investigation that produced it. Every expected value in the plugin's `runtime/meter/token-cost.test.mjs` was computed by hand from these files, with the arithmetic written beside it in the test, not by running the meter. They moved here from `scripts/fixtures/transcripts/` when the meter moved into the workflow plugin, unchanged. They sit outside `runtime/` on purpose: the IT allow list check vouches for every file in a code folder as code it reads, and sample transcripts are data.

The lines of a `.jsonl` file are longer than the runtime's 160 character line rule: one JSON record is one line, and a record cannot be wrapped. That rule is enforced on `.mjs` files, and these are sample input.
