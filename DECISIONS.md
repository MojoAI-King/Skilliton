...
| O28 | The audit's six rules have been measured against this repository and its fixtures only. Over every file this repository has ever changed they report 41 findings, each one a rule's own pattern, a test that plants one, or the defence that contains it (B45), and over the 47 files one wave changed they report 0. What a repository that is not this one looks like to them, and therefore whether the rule set is the right one, is unknown (batch 09-02; lesson `2026-09-21-an-audit-finds-its-own-rules-written-dow-cc3f`) | OPEN | Run `skilliton audit` over the whole tree of the first prepared repository that is not this one, before its delivery policy turns the gate on, and record both counts in docs/COVERAGE.md beside these two |
