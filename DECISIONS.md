...
| O25 | The team compaction window of 600000 in `templates/project-settings.json` is set but never observed. This repository's build sessions compacted automatically 17 times up to 2026-09-20 and no boundary was near it: just short of a million until 2026-09-17, then steadily 164741 to 179051 from 2026-09-18T21:54 onward, including sessions that began after the setting was committed. Whether a project-level value is read for this setting at all is unknown, and no cause is asserted (decision `2026-09-18-the-team-compaction-window-is-600000-own-e7ee`; B36; `evidence/live/2026-09-20-vs-code-extension-session.md` section 5) | OPEN | One session started after the setting was installed, then read the `preTokens` of its first `compact_boundary`: near 460000 means the window is honored, near 170000 means it is not |
| O26 | The lane write guard refuses a lane's writes to the integration branch's own paths only when they come through the assistant's file-writing tool calls. A script writing inside a lane through the shell is not intercepted: `guard-bash.sh` leaves early unless the command mentions git or a removal (`rm`, `rmdir`, `mv`, since guardrails 0.6.0), and the new hook is a `PreToolUse` matcher on `Write` and `Edit` (decision `2026-09-20-the-merge-back-half-of-dispatch-the-lane-ef33`; the limit is stated in the hook's header and in docs/CONTRACTS.md) | OPEN | Decide after the first real dispatch (B38) whether a lane writing through the shell is worth covering in `guard-bash.sh`, or whether the merge-back's conflict report is where that lands |
| O27 | The dispatch suggestion ships on `UserPromptSubmit`, an event read from the hooks reference and never seen delivered to a **plugin** hook here. Everything on Skilliton's side is proved by fixture tests, and the client's side is not (decision `2026-09-20-the-client-event-behind-each-of-the-thre-f6c2`; docs/CLIENTS.md row, batch 07-04 item 2) | OPEN | One live session in the owner pass with the plugin installed and a prompt of six or more items. If the event does not reach a plugin hook, the routine becomes instructed in the harness template (B42) and the hook registration comes out |
| O28 | The audit's six rules have been measured against this repository and its fixtures only. Over every file this repository has ever changed they reported 41 findings, each one a rule's own pattern, a test that plants one, or the defence that contains it; B45 gave each of those lines an allow marker with a reason, and since 2026-09-21 the whole-tree run reports 0 findings over 544 files with 45 allowed lines, matching a scoped run. What a repository that is not this one looks like to them, and therefore whether the rule set is the right one, is unknown (batch 09-02; lesson `2026-09-21-an-audit-finds-its-own-rules-written-dow-cc3f`) | OPEN | Run `skilliton audit` over the whole tree of the first prepared repository that is not this one, before its delivery policy turns the gate on, and record both counts in docs/COVERAGE.md beside these two |
