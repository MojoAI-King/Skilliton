...
| O24 | CLOSED 2026-09-16. After Claude Code and Codex, the owner chose Grok. The owner uses Grok through a Cursor account, not a SuperGrok or X Premium Plus subscription, so the M13 second-vendor spike is Cursor with a Grok model (live sessions on that account). Grok Build, xAI's own command, follows with checks that need no login; its live sessions wait for a subscription. Further tools are ranked after the spike. | CLOSED | docs/PHASE-3.md order and Tool support |
