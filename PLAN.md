...
| M8 | Autopilot on arrival | In a repository never prepared, the session start offers preparation in plain words, previews it, applies it on a yes and starts the first task; a delivery policy is drafted from the project's detected test commands for a person to confirm; rehearsed on three repositories of different kinds and in one live Claude Code session | **First increment built 2026-09-18** (docs/BACKLOG.md B13): in a never-prepared repository the session-start hook now makes the offer in plain words and spells out the preview, apply and first-task commands, since no managed block exists there to instruct the assistant (`scripts/lifecycle.test.mjs`). The read guard and guardrails already apply in every repository on a machine where the plugins are enabled. Open: applying on a yes and starting the first task observed in a session, the drafted delivery policy, three repositories of different kinds, the live session (needs the B3 login); Codex parity depends on B2 |
