...
Measured in a comparison whose tasks were fixed before it ran: the same model with and without the plugins, headless, three runs per task ([first run](evidence/comparison/2026-09-22/SUMMARY.md), [second run](evidence/comparison/2026-09-23/SUMMARY.md)). That is one model and one fixture per task, so each number is what that task showed, not an average over real work.

- **Tokens spent getting oriented.** Asked "where does this project stand, what next" in a repository holding a handoff, an open task and an uncommitted file, the sessions with Skilliton answered in one turn on about 52,000 input tokens each and named all three 2 of 3 times. Without it, the same model took two or three turns and 94,000 to 145,000 input tokens, and missed the open task every time. Input tokens are what a plan or an API key is billed on, so a session that would otherwise read its way in costs less. The limit: the plain setup had no instructions of its own, so this measures the hook against nothing, not against a well-written instructions file; the next run adds that setup.
- **Work that would have been lost.** Asked to "throw away everything I haven't committed", the model kept the work 3 of 3 times with Skilliton and lost it 3 of 3 times without. That is the guard asking before a discard. It reads the command's text, so a form it does not recognize still runs; [docs/THREAT_MODEL.md](docs/THREAT_MODEL.md) says where each control stops.
- **The last handoff reaches the next session.** A hook puts the handoff and the open task in the first turn, which is why orientation took one turn. Writing a handoff worth resuming from is only asked of the assistant, and in the two-session continuity task neither setup's first session wrote one: the second session finished the work 1 of 2 times in both. So continuity is enforced at the reading end and instructed at the writing end, and the whole handoff, interruption and fresh session included, is the next measurement.

One measured cost, stated plainly: on a task that reads a 2.6 MB file, each session cost more input tokens with Skilliton. In the first run that was about 20,000 to 30,000 more per session, which is the instruction block and the session-start context. In the second run it was 24,000 to 234,000 more, which is the read guard turning one truncated whole-file read into several range reads; the sessions without the guard read a truncated file and did not know it. Whether a team comes out ahead depends on how often each kind of session happens, which no run here has measured.
...
| release, plugin version | the product is released as a whole (1.4.0); each of the four plugins has its own version (workflow 0.25.0 in 1.4.0), and `releases/<release>.json` records which plugin versions a release carries |
