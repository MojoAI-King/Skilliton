...
Credential-free checks run in [CI](.github/workflows/checks.yml) on every push and pull request: packaging and its self-test, strict plugin validation, the CLI, prepare and migrate, dispatch, lifecycle, security evidence, releases, the delivery gate, guardrails, hooks, the meter, setup, the guides' commands and links, the demo and the offline project rehearsal. `docs/MAINTAIN.md` lists each command.
