...
- **Dangerous commands are stopped by a hook.** The common forms of force-pushing a protected branch, skipping git hooks, committing a secret-shaped file and deleting the project's records are blocked. Nothing else waits on you: a command the hook can make safe is refused with the fix (throwing away uncommitted work: commit or stash it first), one it cannot read runs and is noted in the git folder, and it asks only before a rule is turned off or saved work is dropped for good (`guardrails.mode`: `strict` asks every time, as before 0.12.0). It is the common forms, not every form: the hook reads command text, and [SECURITY.md](SECURITY.md#what-the-guard-is) says what it cannot see and where the real boundary is.
...
| A machine can prove what it runs | Each release (1.5.0 the latest) is an SSH-signed tag over a manifest that hashes every installed file; `skilliton verify` reads VERIFIED on an install that matches it, and a changed file reads TAMPERED with the file named. On 2026-09-24 it named the one hook file another session had edited in place on this machine, and read all 8 installs VERIFIED once release 1.4.3 replaced that edit (`scripts/release.test.mjs`; [evidence](evidence/live/2026-09-24-release-1.4.2.md), [then](evidence/live/2026-09-24-release-1.4.3.md)) |
...
| release, plugin version | the product is released as a whole (1.5.0); each of the four plugins has its own version (workflow 0.26.0 in 1.5.0), and `releases/<release>.json` records which plugin versions a release carries |
