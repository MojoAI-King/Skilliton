...
3. Run `node scripts/setup.mjs` (show only), then `--apply`, start one session, and show me ~/.claude/skillgate/statusline-keys-seen.log and the last line of ~/.claude/skillgate/usage-log.jsonl. Report whether rate_limits.five_hour and .seven_day were present. Then run `--undo`, confirm settings.json is byte-identical to the backup, then `--apply` again and leave it on so Tier 1 logs for the rest of the week. Separately, read the per-model weekly percentage from /usage by hand and record it in docs/USAGE_BASELINE.md; the status line cannot see that one.
