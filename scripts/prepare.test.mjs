...
function fixture(t) {
  const base = mkdtempSync(join(realpathSync(tmpdir()), "skilliton-prepare-"));
...
test("apply creates the full layout-3 set; check is read-only (prototype cases: apply prepares core records; archives exist)", (t) => {
  const ctx = fixture(t);
  ...
  assert.ok(readFileSync(join(ctx.dir, ".skilliton/security/catalog.json")).equals(CATALOG_BYTES), "the catalog is the package's current catalog, byte for byte");
  assert.match(read(ctx, ".gitignore"), /^\/\.skilliton\/prepare\.lock$/m);
  ...
  assert.match(read(ctx, "docs/security/README.md"), /skilliton security status/);
  assert.match(read(ctx, ".skilliton/security/records/README.md"), /skilliton security status/);
  ...
  for (const rel of LAYOUT_FILES.slice(0, 9)) assert.doesNotMatch(read(ctx, rel), /\b20\d\d-\d\d-\d\d\b/, `${rel} invents no date`);
  assert.doesNotMatch(read(ctx, "docs/MAINTAIN.md"), /<!-- skilliton:/, "the maintain record has no managed block");

  const before = snapshot(ctx.dir, { times: true });
  const check = prepare(ctx, "--check");
  assert.equal(check.code, 0, check.all);
  assert.match(check.out, /\nSummary: prepared \(layout 3\); nothing missing or outdated\./);
...
test("a repeat apply changes zero bytes and keeps human edits (prototype case: reapply is byte-for-byte idempotent)", (t) => {
  ...
  const before = snapshot(ctx.dir, { times: true });
  const backupCount = () => (existsSync(join(ctx.dir, ".git", "skilliton-backups")) ? readdirSync(join(ctx.dir, ".git", "skilliton-backups")).length : 0);
  const backups = backupCount();
  const r = prepare(ctx, "--apply");
  assert.equal(r.code, 0, r.all);
  assert.match(r.out, /\nSummary: already prepared \(layout 3\); nothing to change, nothing written/);
...
test("records in conventional and configured locations are adopted untouched; unknown config keys survive (prototype case: adopts established paths)", (t) => {
  ...
  write(ctx, ".skilliton/config.json", `${JSON.stringify(config, null, 4)}\n`);
  ...
  const originalConfig = readFileSync(join(ctx.dir, ".skilliton/config.json"));
  ...
  const [backup] = readdirSync(join(ctx.dir, ".git", "skilliton-backups"));
  assert.ok(readFileSync(join(ctx.dir, ".git", "skilliton-backups", backup, "CLAUDE.md")).equals(Buffer.from("# Existing rules\nKeep these instructions.\n")));
  assert.ok(readFileSync(join(ctx.dir, ".git", "skilliton-backups", backup, ".skilliton", "config.json")).equals(originalConfig));
...
test("malformed config refuses without partial output or quoting the file (prototype case)", (t) => {
  const ctx = fixture(t);
  write(ctx, ".skilliton/config.json", "{ private-invalid-content");
...
test("symlinked, hard-linked and traversal paths refuse with no writes (prototype cases: symlinked destination, traversal)", (t) => {
  ...
  const attempt = (setup, message, label) => {
    ...
    assert.equal(existsSync(join(ctx.dir, ".git", "skilliton-backups")), false, `${label}: no backup made`);
  ...
  attempt(() => symlinkSync(ctx.outside, join(ctx.dir, ".skilliton")), /symbolic link/, ".skilliton folder is a link");
  ...
  attempt(() => write(ctx, ".skilliton/config.json", JSON.stringify({ prepare: { artifacts: { status: "../outside.md" } } })), /repository-relative/, "a traversal path");
  attempt(() => write(ctx, ".skilliton/config.json", JSON.stringify({ prepare: { artifacts: { status: "same.md", backlog: "same.md" } } })), /two record roles/, "two roles on one file");
});

test("a linked backup folder inside the Git folder is refused before anything is written", (t) => {
  const ctx = fixture(t);
  symlinkSync(ctx.outside, join(ctx.dir, ".git", "skilliton-backups"));
...
test("reserved and case-alias record paths cannot replace instruction files (prototype case)", (t) => {
  ...
  for (const artifacts of [{ status: "docs/security/README.md" }, { status: "agents.md" }, { status: "same.md", backlog: "SAME.md" }, { status: ".GIT/config.md" }]) {
    write(ctx, ".skilliton/config.json", JSON.stringify({ prepare: { artifacts } }));
    const before = snapshot(ctx.dir);
    const r = prepare(ctx, "--apply");
    assert.equal(r.code, 2, r.all);
...
test("lock contention refuses; a subfolder or a folder outside Git is refused (prototype case)", (t) => {
  const ctx = fixture(t);
  write(ctx, ".skilliton/prepare.lock", "another process");
  const before = snapshot(ctx.dir);
  const r = prepare(ctx, "--apply");
  assert.equal(r.code, 2, r.all);
  assert.match(r.err, /prepare\.lock exists/);
  assert.deepEqual(snapshot(ctx.dir), before);
  rmSync(join(ctx.dir, ".skilliton"), { recursive: true });
...
test("a layout-1 project is refused with the migrate command; check reports it as attention", (t) => {
  ...
  assert.match(r.err, /skilliton migrate --dir /);
  const check = prepare(ctx, "--check");
  assert.equal(check.code, 1, check.all);
  assert.match(check.out, /Summary: setup incomplete: .*skilliton migrate/);
  assert.deepEqual(snapshot(ctx.dir), before);
});

test("a project requiring a newer workflow runtime is refused", (t) => {
  const ctx = fixture(t);
  write(ctx, ".skilliton/config.json", JSON.stringify({ prepare: { requires: { workflow: "99.0.0" } } }));
  const before = snapshot(ctx.dir);
  const r = prepare(ctx, "--apply");
  assert.equal(r.code, 2, r.all);
...
test("a missing or outdated managed file makes check incomplete, and apply repairs only that (prototype case: missing runtime)", (t) => {
  const ctx = fixture(t);
  assert.equal(prepare(ctx, "--apply").code, 0);
  rmSync(join(ctx.dir, ".skilliton/security/catalog.json"));
  writeFileSync(join(ctx.dir, "AGENTS.md"), "# Local notes\n");
  const records = Object.fromEntries(LAYOUT_FILES.slice(0, 9).map((rel) => [rel, readFileSync(join(ctx.dir, rel))]));
  const before = snapshot(ctx.dir, { times: true });
  const check = prepare(ctx, "--check");
  assert.equal(check.code, 1, check.all);
  assert.match(check.out, /create\s+\.skilliton\/security\/catalog\.json/);
...
test("an existing catalog is kept when usable and refused when not; a copied runtime is refused (prototype cases: incompatible catalog and runtime; invalid catalog)", (t) => {
  const ctx = fixture(t);
  assert.equal(prepare(ctx, "--apply").code, 0);
  const catalogPath = join(ctx.dir, ".skilliton/security/catalog.json");
  ...
  assert.match(kept.out, /adopted\s+\.skilliton\/security\/catalog\.json/);
...
test("an injected write failure rolls back and leaves no addable backup files (prototype case)", (t) => {
  const ctx = fixture(t);
  write(ctx, ".skilliton/config.json", "{}\n");
  write(ctx, ".gitignore", "/.skilliton/config.json\n");
  const before = snapshot(ctx.dir);
  ...
  const backups = readdirSync(join(ctx.dir, ".git", "skilliton-backups"));
  assert.equal(backups.length, 1);
  assert.equal(readFileSync(join(ctx.dir, ".git", "skilliton-backups", backups[0], ".gitignore"), "utf8"), "/.skilliton/config.json\n");
});

test("an edit arriving during backup is preserved and prepare refuses (prototype case)", (t) => {
  const ctx = fixture(t);
  write(ctx, "AGENTS.md", "# Original\n");
  const r = sg(ctx, ["prepare", "--dir", ctx.dir, "--apply"], { preload: preloader(ctx, EDIT_AFTER_AGENTS_BACKUP), env: { SKILLITON_TEST_TARGET: ctx.dir } });
  assert.equal(r.code, 2, r.all);
...
syncBuiltinESMExports();`);
  const r = sg(ctx, ["prepare", "--dir", ctx.dir, "--apply"], { preload: inject, env: { SKILLITON_TEST_TARGET: ctx.dir } });
  assert.equal(r.code, 3, r.all);
  ...
  assert.equal(existsSync(join(ctx.dir, ".skilliton/prepare.lock")), false, "the lock is released after a failure");
});
...
test("--json prints exactly one result object with the exit code's meaning", (t) => {
  ...
  write(ctx, ".skilliton/config.json", "[1, 2]");
  ...
  assert.deepEqual(snapshot(ctx.dir)[".skilliton/config.json"], Buffer.from("[1, 2]").toString("base64"));
});
