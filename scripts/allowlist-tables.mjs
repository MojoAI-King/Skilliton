...
export const DYNAMIC_CALLS = [
  ...
  { file: `${WORKFLOW}/runtime/lib/awake.mjs`, callee: "spawnSync", arg: "program", count: 2, programs: ["pmset", "sysctl"], why: "on macOS only and only for a failing run: lastSleepMs reads /usr/sbin/sysctl -n kern.sleeptime, and sleptBetween reads /usr/bin/pmset -g log only when that last sleep began after the step did, to name a sleep inside it (B91)" },
