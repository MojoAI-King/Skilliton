...
const env = isolatedEnv(ws, { SKILLITON_DENYLIST: denylist });
...
const sg = (args, opts = {}) => run(process.execPath, [join(fork, "scripts", "skilliton.mjs"), ...args], { env, ...opts });
...
const installedPath = (plugin) => readJson(join(cfgB, "plugins", "installed_plugins.json")).plugins[`${plugin}@skilliton`]?.[0]?.installPath;
const installedVersion = (plugin) => readJson(join(cfgB, "plugins", "installed_plugins.json")).plugins[`${plugin}@skilliton`]?.[0]?.version;
...
await R.step("C1", "clean Claude Code configuration installs the company plugins and verifies them", () => {
  ...
  const installs = ["workflow", "guardrails", "context-hygiene"].map((p) => [p, cc(["plugin", "install", `${p}@skilliton`]).code]);
  const v = verifyB();
...
await R.step("X1", "clean Codex home installs the company plugins and verifies them", () => {
  ...
  const installs = ["workflow", "guardrails", "context-hygiene"].map((p) => [p, cx(["plugin", "add", `${p}@skilliton`, "--json"]).code]);
...
await R.step("P1", "a new application is prepared with the installed plugin's own runtime", () => {
  ...
  writeFileSync(join(app, "CLAUDE.md"), "# App\n\nHuman notes that Skilliton must keep.\n");
  commitAll(app, "App", ["package.json", "price.js", "price.test.js", "CLAUDE.md"]);
  const bin = join(installedPath("workflow"), "bin", "skilliton");
  const prep = run(bin, ["prepare", "--dir", app, "--apply"], { env });
  const check = run(bin, ["prepare", "--dir", app, "--check"], { env });
  const kept = readFileSync(join(app, "CLAUDE.md"), "utf8").includes("Human notes that Skilliton must keep.");
  if (prep.code === 0) git(app, ["add", "-A"], { env }), git(app, ["commit", "-q", "-m", "Prepare with Skilliton"], { env });
...
async function runEval(label) {
  ...
  const e = run(claude.path, ["plugin", "eval", join(fork, "packs", "base", "plugins", "workflow"), "--case", EVAL_CASE, "--runs", "1", "--ablation", "none", "--scaffold", "--trust-plugin", "--no-publish", "--output-dir", out], { env: { ...process.env, SKILLITON_DENYLIST: denylist }, timeoutMs: 900000 });
...
await R.step("I1", "the improvement: review skill rule, template sentence, eval case, version bump, offline checks", async () => {
  ...
  const checks = ["packs.test.mjs", "skilliton.test.mjs"].map((t) => [t, run(process.execPath, [join(fork, "scripts", t)], { cwd: fork, env })]);
...
await R.step("U1", "the Claude Code environment receives the improved skill and verifies it", () => {
  const mu = cc(["plugin", "marketplace", "update", "skilliton"]);
  const pu = cc(["plugin", "update", "workflow@skilliton"]);
...
await R.step("U2", "the Codex environment receives the improved skill and verifies it", () => {
  ...
  const up = cx(["plugin", "marketplace", "upgrade", "skilliton"]);
  const upExpected = up.code === 0 || /not configured as a Git marketplace/.test(up.all);
  const add = cx(["plugin", "add", "workflow@skilliton", "--json"]);
  const cacheDir = join(codexC, "plugins", "cache", "skilliton", "workflow");
...
await R.step("M1", "the project applies the release's migration, rolls it back, and applies it again, keeping human text", () => {
  ...
  const receipts = existsSync(join(app, ".skilliton", "migrations")) ? readdirSync(join(app, ".skilliton", "migrations")) : [];
  const text = readFileSync(join(app, "CLAUDE.md"), "utf8");
  const refreshed = text.includes("needs the payments lead's review before it merges") && text.includes("A note a person added after preparing.") && text.includes("Human notes that Skilliton must keep.");
...
await R.step("T1", "a tampered install is reported TAMPERED with the file named, and a reinstall restores VERIFIED", () => {
  ...
  const un = cc(["plugin", "uninstall", "guardrails@skilliton"]);
  const re = cc(["plugin", "install", "guardrails@skilliton"]);
  const v2 = verifyB();
...
await R.step("A1", "an unauthorized release (unsigned bump with a changed hook, plus a tag signed by an untrusted key) reaches the environment and is not VERIFIED", () => {
  ...
  const tag = run("git", ["-c", "gpg.format=ssh", "-c", `user.signingkey=${join(keys, "intruder-example-invalid")}`, "tag", "-s", `skilliton-release/9.9.9`, "-m", "skilliton release 9.9.9", "-m", "manifest-sha256: 0000"], { cwd: fork, env: intruderEnv });
  const list = sg(["release", "list", "--company", "acme"], { cwd: fork });
  cc(["plugin", "marketplace", "update", "skilliton"]);
  const pu = cc(["plugin", "update", "workflow@skilliton"]);
  const version = installedVersion("workflow");
  const v = verifyB();
  const j = jsonResult(v);
...
await R.step("R1", "rollback: the company removes the bad tag and reverts; the environment downgrades and verifies", () => {
  git(fork, ["tag", "-d", "skilliton-release/9.9.9"], { env });
...
await R.step("D1", "removal: plugins uninstalled, managed content removed, records and history kept", () => {
  ...
  const uninstalls = ["workflow", "guardrails", "context-hygiene"].map((p) => cc(["plugin", "uninstall", `${p}@skilliton`]).code);
  const rm = sg(["remove", "--dir", app, "--apply"]);
  const claudeMd = readFileSync(join(app, "CLAUDE.md"), "utf8");
  const blockGone = !claudeMd.includes("skilliton:harness:start");
  const humanKept = claudeMd.includes("Human notes that Skilliton must keep.");
  const recordsKept = existsSync(join(app, "docs", "lessons")) && existsSync(join(app, ".skilliton", "migrations"));
