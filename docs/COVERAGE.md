...
- Not exercised: a live Claude Code session refusing a Read (the deny shape is measured for Bash only), a session compacting at the team settings' `autoCompactWindow`, either hook on Windows (docs/WINDOWS.md), and Codex (hooks in plugins are removed there).
