...
- Every git command the runtime makes is run with the settings a repository or the environment could otherwise hand it removed: no repository configuration for the reachability check (it runs with `GIT_DIR` pointing at an empty folder), the `ext` transport refused, and `GIT_CONFIG`, `GIT_CONFIG_GLOBAL`, `GIT_CONFIG_SYSTEM`, `GIT_CONFIG_COUNT` and its keys, `GIT_PROXY_COMMAND`, `GIT_SSH_COMMAND`, `GIT_ALLOW_PROTOCOL`, `GIT_EXTERNAL_DIFF`, `GIT_TEXTCONV` and `GIT_EXEC_PATH` taken out of the environment, together with the variables that name a program for git to run for a person (`GIT_ASKPASS`, `SSH_ASKPASS`, `GIT_EDITOR`, `GIT_PAGER`, `GIT_TEMPLATE_DIR`) and the ones that would name a different repository (`GIT_DIR`, `GIT_INDEX_FILE` and the rest), so that what a check reports is the repository it was pointed at. A copied working folder carries its own `.git/config`, and that file can rewrite an address and name a command to run for it; a prepared project's settings file can put variables into a session's environment, which is the same thing by another route. The guardrails hook takes the same variables out of its own git calls.
- The one command that reaches a network, the reachability check, also takes its home folder from the system's record of this user rather than from `HOME`, and ignores `XDG_CONFIG_HOME`. `HOME` chooses `~/.gitconfig`, and that file can rewrite an address and name a program to run for it, so on that one call the configuration that applies is this machine's own and not one a project put in the environment. Where the system has no record for this user (a container started with an unknown user id), the check says so in its own line rather than claiming a pinned home. What this machine's own `~/.gitconfig` and `/etc/gitconfig` say is followed on purpose, on that call and on every other: the company proxy and certificate live there, and the clone this check is a promise about would use them too.
