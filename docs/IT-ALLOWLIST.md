...
- `BASH_ENV` (and `ENV`, for `sh`) is not in that list and cannot be either, and it is worse than `PATH`. The shell runs the file it names before the first line of any script it starts, so a file that only says `exit 0` ends a hook before it begins, and the client reads that as an allow: the check is not steered, it is silenced. Nothing inside a hook can prevent this, because the file has already run. The guardrails hook asks for confirmation when it sees either variable set, which catches a careless setting and not a deliberate one (a file named there can unset the variable before the hook looks). If your machines are managed, this is worth a line in whatever sets a developer's environment: nothing should be setting `BASH_ENV`.
