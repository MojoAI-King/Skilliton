...
- A program that never returns is reported as not checked rather than as blocked, and the check moves on. The programs the hooks use are started by one script with one timeout; a program that hangs is stopped with its own children, and the run says so.
