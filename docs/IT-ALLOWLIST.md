...
- Every git command the runtime makes is run with the settings a repository or the environment could otherwise hand it removed: no repository configuration for the reachability check (it runs with `GIT_DIR` pointing at an empty folder), the `ext` transport refused, and `GIT_CONFIG`, `GIT_CONFIG_GLOBAL`, `GIT_CONFIG_SYSTEM`, `GIT_CONFIG_COUNT` and its keys, `GIT_PROXY_COMMAND`, `GIT_SSH_COMMAND`, `GIT_ALLOW_PROTOCOL`, `GIT_EXTERNAL_DIFF` and `GIT_TEXTCONV` taken out of the environment, together with the variables that would name a different repository (`GIT_DIR`, `GIT_INDEX_FILE` and the rest), so that what a check reports is the repository it was pointed at. A copied working folder carries its own `.git/config`, and that file can otherwise rewrite an address and name a command to run for it; a prepared project's settings file can put variables into a session's environment, which is the same thing by another route. What is deliberately left in place is this machine's own git configuration (`~/.gitconfig` and `/etc/gitconfig`): the company proxy and certificate live there, and the reachability check exists to say whether this machine, set up as it is, can reach the company repository.
