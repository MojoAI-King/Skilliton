...
- **The matching test:** `scripts/allowlist.test.mjs` (with `scripts/inventory.mjs`, which reads the shell and JavaScript code), and `scripts/footprint.test.mjs` for the four promises in the short version. Each has a `--self-test` that proves its checks fail on known-bad input. What an interpreter is given is read too: a file a script hands to `bash`, `sh`, `node` or `python3` has to be a file those tests read, and a program written into a script, or a path built from a variable, is listed with what it runs.
