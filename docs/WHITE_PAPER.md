...
|---|---|---|
| `workflow` | Session start: the last handoff and a project state block. Stop: a checkpoint reminder when the tree changed and none was recorded. Pre-compact and session end: a journal entry. Prompt submit: the dispatch suggestion on a long list. | `task`, `review`, `handoff`, `maintain`, `security`, `dispatch` |
| `guardrails` | Every shell command: deny force-push to protected branches, skipped git hooks and secret-shaped files; ask before commands that discard uncommitted work; ask on a shell string that names git. Every file write: the lane write guard inside a dispatched lane. | `guardrails` (explains a block) |
