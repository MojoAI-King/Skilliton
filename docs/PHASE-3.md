...
- **An IT allow list and a preflight check.** Companies run endpoint security such as application allowlisting (ThreatLocker and similar), endpoint detection and response, and proxies that inspect encrypted traffic. [IT-ALLOWLIST.md](IT-ALLOWLIST.md) lists every program Skilliton starts, every folder it writes, its network use and the privileges it needs, so IT can write rules before rollout, and `scripts/allowlist.test.mjs` keeps that page matched to the code by reading the code. `skilliton preflight` runs the same list on a laptop before anything is set up and names anything blocked with what to allow; `join` runs it first and refuses, with nothing written, when something would stop setup. `scripts/footprint.test.mjs` keeps the footprint small enough for the list to stay short: user-level only after IT places the managed files, no downloaded executables, no network of its own beyond one reachability check, nothing left running. Built 2026-09-17 (B26, B27, B28); proved against blocks made on purpose on a real machine, and **not tested under any endpoint-security product** (B29), because none is available.
...
**Measured on Linux, 2026-09-17** ([enrollment rehearsal](../evidence/rehearsals/2026-09-17-enrollment/SUMMARY.md): Claude Code 2.1.274, a clean container, no login; session starts used a placeholder key and a model endpoint that never answers). A drop-in placed by root does install the company plugins with no developer command, but not at once. From the GitHub marketplace, the first session start registers it, the second installs the plugins as managed, and only the third runs with their skills and hooks. From a marketplace folder on the machine, the second start is active. A read-only plugin seed placed by root is active from the second start and never auto-updates. Running the install commands `join` uses, as the person and before their first session, makes the first start active with no login, and Claude Code then holds the plugins as managed. So the bundle pairs the drop-in with a first-login install (decision 2026-09-17-enrollment-installs-claude-code-plugins-9cf3). Not measured: a claude.ai login, an interactive first session after logging in, and a clean macOS account, which needs a separate Mac or a macOS virtual machine because managed settings on macOS apply to the whole machine.

**Acceptance.** On clean machines, with files placed where and as device management places them: the first login ends VERIFIED on each configured tool with no developer command; a tampered file makes the detection script report TAMPERED; offboarding leaves nothing Skilliton added; a pinned ring does not follow a newer release. A real Intune or Jamf tenant needs the owner's access and is recorded separately. The same first login also ends VERIFIED under a real endpoint-security policy (a trial or company test tenant, recorded separately), the preflight check names each blocked item in a policy that blocks one on purpose, Windows is either supported and rehearsed or stated as unsupported, and install, update and verify work from a private company repository.
...
| 2 | M12 spike: a Claude Code drop-in on a clean container (measured 2026-09-17) and a clean macOS account (not run: needs a separate Mac or a macOS virtual machine) | Tests the riskiest assumption while changing course is still cheap |
...
|---|---|---|
| A company can push its Skilliton package to every device the way Intune pushes a laptop setup | **TRUE for delivery** (documented for Claude Code: managed settings files, macOS profiles, Windows registry, with Jamf and Intune templates) | Skilliton builds the bundle; the company's device management delivers it. Measured on Linux (2026-09-17): the drop-in installs the plugins with no developer command, active from the third session start, or from the first with a first-login install; macOS and a real login not yet measured |
...
- **Where Skilliton stops (added 2026-09-17):** Skilliton is the harness for each developer working with an AI assistant, in line with company policy and practices, which also lets a company put more creative people into coding roles. Its part in team work is the company package, the shared records and the merge gate. A live environment where a team watches and steers the same agent session is a separate companion project with its own repository, meant to work alongside Skilliton; its idea, open questions and brief are kept in that project's repository. No milestone here builds it.
...
| An endpoint-security product to test under (a ThreatLocker trial or a company test tenant) | M12 | Not available as of 2026-09-17. `skilliton preflight` is proved against blocks made on purpose on a real machine (a program taken off PATH, one that is there but may not run, one whose exec fails, an unwritable folder, a plugin folder whose scripts may not run, a refused connection); a run under a product stays not measured |
| Whether Windows is in scope, and how (PowerShell versions of the scripts, or requiring WSL or Git Bash) | M12, M13 | Decided 2026-09-17: in scope, and supported through Git for Windows, with one implementation of every hook (decision cb9e). Built the same day and unmeasured; the owner runs [WINDOWS.md](WINDOWS.md) on the Windows machine |
