...
- **An IT allow list and a preflight check.** Companies run endpoint security such as application allowlisting (ThreatLocker and similar), endpoint detection and response, and proxies that inspect encrypted traffic. [IT-ALLOWLIST.md](IT-ALLOWLIST.md) lists every program Skilliton starts, every folder it writes, its network use and the privileges it needs, so IT can write rules before rollout. A preflight check (planned) runs the same list on a laptop and names anything a policy blocks, instead of the tools failing without a word. Skilliton keeps its footprint small so the list stays short: user-level only after IT places the managed files, no downloaded executables, no web requests of its own, nothing in system folders.
