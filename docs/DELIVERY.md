...
On the machine that holds the shared bare repository: a git version that verifies SSH commit signatures (`gpg.format ssh`, `gpg.ssh.allowedSignersFile`), OpenSSH's `ssh-keygen`, `tar`, Node.js (22 or later, the version CI runs), and the Skilliton workflow plugin (an installed copy or a checkout of your company skills repository). Only git 2.51 and Node.js 25 on one macOS machine have been exercised (see "What has been verified"); minimum versions are not established.
