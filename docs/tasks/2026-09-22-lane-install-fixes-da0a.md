...
- [ ] N37. [TOUCH] `lib/fork.mjs` `validateRepo` quotes its value verbatim, so `company init --marketplace-repo 'TOKEN@github.com/o/r'` prints a token typed as a user name: pass the value through the existing `redact()` from lib/preflight.mjs before it reaches any message. Test that a planted value is not printed.
- [ ] N38. [TOUCH] The context-hygiene session-start line "SKILLITON_LESSONS not set or file missing; injecting nothing" reads as an error to a newcomer on every session: packs/base/plugins/context-hygiene/hooks/session-start-checklist.sh: print nothing when SKILLITON_LESSONS is unset (an optional personal feature that was never set up), and keep a line, reworded to name the path it could not read, when it is set but the file is missing or unreadable. Test both cases.
