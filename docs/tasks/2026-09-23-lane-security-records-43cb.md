...
- **Owner:** unassigned
- **Updated:** 2026-09-24T03:51:56.163Z
...
- **State:** N12 done: security findings writes docs/SECURITY_FINDINGS.md (security.findingsFile) and the backlog keeps one linking line with the open count; maintain uses the same writer; migration 0004-security-findings-file in lib/migrations-findings.mjs, registered on the closing line of MIGRATIONS; migrationState does not yet list a same-layout migration as pending (main window). Evidence: node scripts/security-findings.test.mjs 6 of 6; four security suites 66 of 66 after updating three assertions that named the old location; migrate group 76 of 76; rename, lifecycle, release pass; lint, lint-shape (pin 22 to 21), deadcode pass.
- **Next:** N13: assessments for the eleven missing controls
- **Blocked:** nothing
