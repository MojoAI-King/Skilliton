...
- **Owner:** unassigned
- **Updated:** 2026-09-23T19:51:33.166Z
...
- **State:** N83 done: lib/delivery-protect.mjs holds the files a check runs (default) or the policy's protectedPaths; an unsigned change is rejected before the archive is extracted or any check runs. Evidence: node scripts/delivery-protect.test.mjs exit 0 (4 pass); node scripts/delivery.test.mjs exit 0 (15 pass); with the rule disabled the adversary test fails (the push was accepted).
- **Next:** N84 bounded line reader
- **Blocked:** nothing
