...
- **Owner:** unassigned
- **Updated:** 2026-09-24T04:01:40.104Z
...
- **State:** N31 built: skilliton pin moves Claude Code's company marketplace to the release tag (remove, add at <source>#<tag>, read back the ref, update the plugins), every client command from a new empty folder; preview runs nothing. Evidence: scripts/pin-marketplace.test.mjs pin cases 9 of 9 with a stub client; release.test.mjs 27 of 27; lint, lint-shape, deadcode, docs, names, scrub-check pass; allowlist.test.mjs fails only on the three rows main has to add to scripts/allowlist.test.mjs.
- **Next:** N32: join adds the marketplace at the release tag, from a neutral folder
- **Blocked:** nothing
