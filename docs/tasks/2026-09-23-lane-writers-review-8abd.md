...
- **Owner:** unassigned
- **Updated:** 2026-09-23T15:08:26.842Z
...
- **State:** N74 done: the gate log is opened with O_NOFOLLOW after an lstat of each folder below the Git folder, a hard-linked log is refused, and the file is truncated only after the checks on the open descriptor; a refusal is exit 3 naming the link. Evidence: node --test scripts/gate.test.mjs: 19 pass, 0 fail (the new link test fails at the base with exit 0 instead of 3); lint-shape, lint, deadcode, scrub-check exit 0.
- **Next:** N75: the skill writers stay inside the repository
- **Blocked:** nothing
