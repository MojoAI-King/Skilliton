...
- [ ] N27. [TOUCH] HIGH: force-push through pattern refspecs and `heads/`: around lines 805 to 816, `dst` has only `refs/heads/` stripped and is matched literally. Allowed today: `git push -f origin 'refs/heads/*'`, `git push -f origin 'refs/heads/*:refs/heads/*'`, `git push origin '+refs/heads/*:refs/heads/*'`, `git push -f origin HEAD:heads/main` (git expands heads/main to refs/heads/main; a real remote's main was force-updated). Fix: a dst containing `*` hits every protected branch; strip a leading `refs/`, then `heads/`, before is_protected.
- [ ] N28. [TOUCH] MEDIUM: deleting a protected branch on the remote: check_push around lines 756 to 785 allows `git push origin --delete main`, `git push -d origin main` and `git push origin :main`. Treat --delete, -d and an empty source the same as force for protected names.
