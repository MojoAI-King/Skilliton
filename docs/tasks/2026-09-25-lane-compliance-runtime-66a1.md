...
- **Owner:** unassigned
- **Updated:** 2026-09-25T04:47:35.298Z
...
- **State:** N2 done: skilliton compliance scope (port of MojoComply scope.ts: 7 root files plus workspace manifests, 512 KB cap, negation window, 3 hits per signal, 21 detectors, scoring and recording gate) with --json and --apply --decided-by; config compliance section; fixture knowledge base. Evidence: node scripts/compliance-scope.test.mjs: 11 pass, 0 fail, 1 skipped (the shipped scope-kb.json case, until the frameworks-data lane merges); lint, lint-shape, footprint, write-sites, skilliton, docs, names, packs, allowlist, config tests exit 0.
- **Next:** N3: baseline crosswalk and compliance sheet
- **Blocked:** nothing
