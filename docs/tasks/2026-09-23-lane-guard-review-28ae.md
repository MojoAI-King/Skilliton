...
- **Owner:** unassigned
- **Updated:** 2026-09-23T16:12:05.175Z
...
- [x] N73. [FEATURE] The ask rules cover the discard verbs and the deny rule covers the record-deletion forms: guard-bash.sh `check_checkout`, `check_restore`, a new worktree rule, `check_remove`, `protected_target`, SKILL.md: ask on `git checkout -f`, `git checkout -f <branch>`, `git switch -f`, `git switch --discard-changes`, `git switch -C <branch> <start>`, `git worktree remove --force`, `git rm -f <tracked path>`, `git restore <path>`, `git restore -W <path>`, `git checkout -- <path>` (a single modified path is still a discard; asking on every restore is acceptable, the skill says so), `rm -rf .git`; deny `find <records folder> -delete` and `find . -name '*.md' -delete` when the records folder is under the search root, brace expansion (`rm -rf docs/{tasks,decisions}` is read as the two paths), `rm -rf "$PWD/docs"` and `rm -rf ./docs/../docs`, `truncate -s 0 <record file>`, a redirection `> <record file>`, `cp /dev/null <record file>`, a heredoc onto CLAUDE.md or AGENTS.md (the managed block lives there), `rsync --delete` into the records folder, `python3 -c` or `perl -e` text that names `rmtree`, `unlink` or `os.remove` with a record path; and on a case-insensitive filesystem (test with `ls` of a differently cased spelling resolving, or compare names case-folded when the project's disk is case-insensitive, detected once per run) `rm -rf DOCS` and `rm decisions.md` are read as the record paths they hit. Done looks like: every listed command has a case with the base decision and the new one; ordinary `rm` of a non-record file stays allowed; the existing record-protection cases in guardrails.test.sh still pass. Evidence: scripts/guardrails-review.test.sh, the two N73 sections (every listed command with its base decision and the new one, checked against the base hook with no mismatch; the case-insensitive cases ran here on a case-insensitive disk, and a case-sensitive disk expects allow); scripts/guardrails.test.sh still passes (676 ok).
...
### 2026-09-23T15:58:09.414Z

- **State:** N73 done: checkout -f, checkout -- <path>, checkout -B, switch -f/--discard-changes/-C, restore <path>, rm -f, worktree remove --force and rm -rf .git ask; find -delete, braces, $PWD, truncate, > onto, cp onto, heredoc onto CLAUDE.md or AGENTS.md, tee, rsync --delete, program rmtree/unlink/os.remove and case-folded names on a case-insensitive disk deny
- **Evidence:** bash scripts/guardrails-bypass.test.sh exit 0 (143 ok; review file 199 ok, 112 base labels match the base hook); guardrails.test.sh exit 0 (676 ok); allowlist, footprint, git-config, lint, names, audit.test, secret-rules, rename, scrub exit 0
- **Next:** run node scripts/checks.mjs, write LANE_REPORT.md
- **Git:** lane/guard-review-0923 @ 33b47ff, 4 uncommitted

## Handoff

- **State:** N73 done: checkout -f, checkout -- <path>, checkout -B, switch -f/--discard-changes/-C, restore <path>, rm -f, worktree remove --force and rm -rf .git ask; find -delete, braces, $PWD, truncate, > onto, cp onto, heredoc onto CLAUDE.md or AGENTS.md, tee, rsync --delete, program rmtree/unlink/os.remove and case-folded names on a case-insensitive disk deny. Evidence: bash scripts/guardrails-bypass.test.sh exit 0 (143 ok; review file 199 ok, 112 base labels match the base hook); guardrails.test.sh exit 0 (676 ok); allowlist, footprint, git-config, lint, names, audit.test, secret-rules, rename, scrub exit 0.
- **Next:** run node scripts/checks.mjs, write LANE_REPORT.md
- **Blocked:** nothing
