...
- **Owner:** unassigned
- **Updated:** 2026-09-23T15:24:51.951Z
...
- **State:** N70 done: writes to .claude/settings.json or settings.local.json that bring in or meet hooks, disableAllHooks or enabledPlugins ask, and creating .skilliton-off asks. Evidence: bash scripts/guardrails-bypass.test.sh exit 0 (143 ok, runs guardrails-review.test.sh: 42 ok); bash scripts/guardrails.test.sh exit 0 (676 ok); allowlist, lint, names, scrub exit 0.
- **Next:** N71: git option prefixes, --no-force, -c config, unresolved variables, --no-verify on more verbs, hook-skip variables, .git/hooks
- **Blocked:** nothing
