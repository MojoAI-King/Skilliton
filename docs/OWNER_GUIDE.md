...
- Maintenance is half automatic since workflow 0.19.0: `skilliton maintain --apply` does the mechanical part (indexes, security findings, the journal event), and the stop hook asks for it and the judgment part, once per commit, when a merge, 15 commits or a day of commits landed since the last one, or when the handoff is 15 commits behind. The judgment part (decisions and lessons from the conversation, the prose of status and backlog, the handoff) is still the assistant's work; the hook makes sure it is asked, once per commit, without a person remembering.
