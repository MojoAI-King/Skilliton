...
After the batch, 2026-09-25: the owner approved every item that waited on the owner, in one sentence ("I approve all the things you are waiting on me for"). The second kill criterion did not fire: the baseline crosswalk is marked reviewed, with the project owner named as its reviewer, and the sheet now says so. Before the approval was written in, the assistant read the 15 rows against the NIST CSF 2.0 subcategory text: every id exists in the spine and every reason matches what its subcategory asks. One rating is arguable and was left as approved: SG-ACCESS-CONTROL is strong for PR.AA-05, which also asks for least privilege, separation of duties and review, and access rules with deny tests show none of those three. The licence check came back no for both SOC 2 and PCI DSS; that outcome is decision 2026-09-25-the-soc-2-and-pci-dss-libraries-stay-out-8c57. The owner's verdict column in evidence/live/2026-09-25-compliance-two-repositories.md reads agreed for all eight proposals, from the same approval.
