...
the first step's own evidence set includes the file the second step writes to, expect one extra settling run
before assuming "collect once, left alone" holds. Verify multi-run behavior by actually running the command
several times against a disposable fixture rather than reasoning from the diff alone.

## What now enforces it

scripts/maintain-security-collectors.test.mjs's first test runs `maintain --apply` five times in a row against a
fixture with a tracked backlog file and asserts the exact wrote/wrote/current/current sequence (and the
current/current/wrote/wrote/current sequence once a second collector is introduced), so a change that removes the
settling run, or that makes it loop instead of settle, fails the test.
