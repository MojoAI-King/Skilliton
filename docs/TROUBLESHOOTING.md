...
| The stop hook says dispatch was named | A prompt in this session read as six or more separate items, the prompt hook directed `/workflow:dispatch`, and no `LANES.md` was written after it (workflow 0.20.0); asked once per such prompt | Run `/workflow:dispatch`, or tell the user in one line why the items are not separate pieces of work. |
