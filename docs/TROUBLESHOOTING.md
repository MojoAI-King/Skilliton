...
| The state block says another session looks active in this checkout | The journal shows a different session in this checkout, active within the last 30 minutes and with no session-end; an interrupted session cannot be told apart from one still open (workflow 0.22.0) | Check for another open window before resuming. If one is open, work in a worktree lane, or check before committing the shared records. |
...
| The session-start line says a rule is turned off | The project's `.skilliton/config.json` lowers a guardrail | That is the team's setting; the line exists so it is never silent. A command only the turned-off rule would have caught passes with a notice naming the setting (guardrails 0.7.0). |
...
| The stop hook says maintenance is due | A merge or 15 commits landed, or a day and a commit passed, since the last `maintain` event in the journal (workflow 0.19.0; the commit count 0.22.0), or the shared handoff is 15 or more commits behind HEAD (workflow 0.22.0; only committing a handoff clears it); asked once per commit, on an integration branch only | `skilliton maintain --apply`, then the judgment half the message lists (or `/workflow:maintain`). `checkpoints.stopReminder: false` in `.skilliton/config.json` turns both stop reminders off. |
| A notice says guardrails let a command through only because a rule is turned off | `.skilliton/config.json` sets that guardrails rule to false, and this command is one the rule would have refused (guardrails 0.7.0) | Nothing, if the setting is intended. Otherwise set it back to true, or delete the line, and the command is refused again. |
| The stop hook says dispatch was named | A prompt in this session read as six or more separate items, the prompt hook directed `/workflow:dispatch`, and no lane plan (`LANES.md`, or since workflow 0.25.0 a numbered one such as `LANES-8.md`) was written after it (workflow 0.20.0); asked once per such prompt | Run `/workflow:dispatch`, or tell the user in one line why the items are not separate pieces of work. |
...
| The session compacted far below the configured window | The project window in `.claude/settings.json` is honored since 2026-09-22 (O25: compactions at about 568000 under a window of 600000); the earlier compactions near 170000 are not explained (B36) | Nothing to fix; write a checkpoint before long stretches so a compaction loses nothing. |
