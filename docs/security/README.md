...
Catalogs shipped with Skillgate are partial sets of original practice summaries with related references to public frameworks; they are not complete framework assessments. `catalogVersion` in `catalog.json` names the version in use, and a newer catalog reaches this project only through a migration.
