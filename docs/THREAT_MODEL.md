...
Kind: Living. What Skilliton protects, against whom, with which mechanism, and where each mechanism stops. Written 2026-09-23 after three independent cold reviews of `main` at a61a778; the limits they found are listed at the end with the release that carries each fix. SECURITY.md is the short public statement; docs/IT-ALLOWLIST.md is the footprint for endpoint security teams; docs/DELIVERY.md is the shared-branch gate. This page is the one that says what each of those is for.
...
Every line below was reproduced on `main` at a61a778 before it was written here. "Guardrails 0.9.0" and "workflow 0.23.0" are the plugin versions that carry the fix; the release that bundles them is 1.2.0.

| Limit at a61a778 | Fix |
|---|---|
