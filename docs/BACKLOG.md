...
| B19 (M9) | Every current technical name is Skilliton, with a migration for prepared projects and joined machines | open; order 1 in docs/PHASE-3.md; one owner choice (control ID prefix, recommendation keep `SG-`) | PLAN.md M9; docs/BRANDING.md |
...
| B23 (M13) | Any AI coding tool: an adapter and a measured CLIENTS.md column per tool | open; order 5 is a second-vendor spike (VS Code with GitHub Copilot or Cursor, depending on the owner's accounts); the owner ranks the tools (O24) | PLAN.md M13; docs/PHASE-3.md tool support |
