...
| B12 (M7) | One command sets up a machine: the company marketplace, its plugins, the release signers, a source for verify and `skillgate` on the terminal path, ending VERIFIED on Claude Code and Codex; installing from a GitHub source measured | open, no owner input needed | PLAN.md section 7 M7; design recorded in docs/CONTRACTS.md before building |
