...
Kind: Reference. Recorded 2026-09-17 by `scripts/rehearsals/company-release.mjs`. Disposable folders, synthetic projects and throwaway keys only.

- **Claude Code:** 2.1.273 (Claude Code)
- **Codex:** codex-cli 0.154.0-alpha.6.2
- **Behavior eval:** not run (pass --with-eval)
- **Node:** v25.8.1

| Step | Result | Evidence |
|---|---|---|
| F1 company fork with an approver key and a trusted signers file | PASS | fork at 5a67e74; trust add exit 0 |
...
| I1 the improvement: review skill rule, template sentence, eval case, version bump, offline checks | PASS | workflow 0.6.0 to 0.6.1; offline checks packs.test.mjs:0 skilliton.test.mjs:0; behavior eval not run (pass --with-eval) |
| I2 release 1.1.0 created with its evidence and signed | PASS | sign exit 0; list: approved    1.1.0       signed by approver@example.invalid (ED25519 SHA256:jc1q5FZvTAkKOo9ofMqlV3i3SLeMELWlCJYIKp+PWUk); manifest-sha256 fd51827d18de; commit b4fcd99e51b5; approved    1.0.0       signed by approver@example.invalid (ED25519 SHA256:jc1q5FZvTAkKOo9ofMqlV3i3SLeMELWlCJYIKp+PWUk); manifest-sha256 cfb2917ec42f; commit 62bb5c42ee09 |
| U1 the Claude Code environment receives the improved skill and verifies it | PASS | marketplace update 0; plugin update 0; installed 0.6.1; improved rule present: true; verify exit 0 (all 3 company plugin install(s) on this client are VERIFIED against approved releases.) |
| U2 the Codex environment receives the improved skill and verifies it | PASS | marketplace upgrade exit 1 (expected for a local-path marketplace, which Codex reads directly); plugin add 0; installed versions 0.6.1; improved rule present in 0.6.1: true; verify exit 0 (all 3 company plugin install(s) on this client are VERIFIED against approved releases.) |
...
| A1 an unauthorized release (unsigned bump with a changed hook, plus a tag signed by an untrusted key) reaches the environment and is not VERIFIED | PASS | untrusted tag created: true; environment received 0.6.2; verify exit 2, UNKNOWN VERSION reported: true; 9.9.9 never listed approved |
| R1 rollback: the company removes the bad tag and reverts; the environment downgrades and verifies | PASS | revert 0; plugin update 0; installed 0.6.1; verify exit 0 |
...
- unauthorized release: the environment updated to 0.6.2 (plugin update exit 0); verify exit 2; release list exit 2 with the untrusted tag
